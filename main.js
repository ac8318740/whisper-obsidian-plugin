...
var AudioHandler = class {
    ...
    try {
      ...
      if (this.plugin.settings.saveAudioFile) {
        try {
          ...
          const audioFilePath = `${this.plugin.settings.saveAudioFilePath ? `${this.plugin.settings.saveAudioFilePath}/` : ""}${audioFileName}`;
          ...
          new import_obsidian3.Notice("Audio file saved successfully");
        } catch (err) {
          console.error("Error saving audio file:", err);
          new import_obsidian3.Notice("Error saving audio file: " + err.message);
        }
      }
      if (!this.assemblyClient) {
        this.initializeAssemblyAI();
  ...
  async processTranscriptionResult(transcript, savedAudioPath, timestampFileName) {
    if (this.plugin.settings.debugMode) {
...
- Speaker ${utterance.speaker}: `;
        ...
        if (this.plugin.settings.usePostProcessing && transcript.speakerIdentifications) {
          ...
          try {
            ...
            const isOpenAIModel = this.plugin.settings.postProcessingModel.startsWith("gpt-") || this.plugin.settings.postProcessingModel === "o1" || this.plugin.settings.postProcessingModel === "o1-mini";
...
`;
      ...
      if (shouldCreateNewFile) {
        ...
        const filePath = `${this.plugin.settings.createNewFileAfterRecordingPath ? `${this.plugin.settings.createNewFileAfterRecordingPath}/` : ""}${fileName}.md`;
...
var WhisperSettingsTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    if (this.plugin.settings.usePostProcessing) {
      ...
      new import_obsidian5.Setting(modelContainer).setName("Use Custom Model").setDesc("Enable to manually enter a model identifier").addToggle(
        (toggle) => toggle.setValue(this.plugin.settings.useCustomModel).onChange(async (value) => {
      ...
      if (this.plugin.settings.useCustomModel) {
        new import_obsidian5.Setting(modelContainer).setName("Post-Processing Model").setDesc("Enter the model identifier to use for post-processing").addText(
          (text) => text.setPlaceholder("Enter model identifier").setValue(this.plugin.settings.postProcessingModel).onChange(async (value) => {
      ...
      const isAnthropicModel = this.plugin.settings.postProcessingModel.startsWith("claude");
      if (isAnthropicModel) {
        new import_obsidian5.Setting(modelContainer).setName("Anthropic API Key").setDesc("Your Anthropic API key for Claude models").addText(
        ...
        new import_obsidian5.Setting(modelContainer).setName("OpenAI API Key").setDesc("Your OpenAI API key for GPT models").addText(
      ...
      new import_obsidian5.Setting(modelContainer).setName("Post-Processing Prompt").setDesc("The prompt to use for post-processing. For AssemblyAI transcripts, include instructions for mapping speaker labels to names.").addTextArea(
      ...
      new import_obsidian5.Setting(modelContainer).setName("Keep Original Transcription").setDesc("Include the original transcription (before post-processing) in the output file").addToggle(
...
var DEFAULT_SETTINGS = {
  ...
  // Default value for useCustomModel
};
